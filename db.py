...
def show_entries():
    cur = g.db.execute(
        'select title, text, time from entries order by id desc')
    entries = [dict(title=row[0], text=row[1], datetime=row[2])
        ...
        if file and allowed_file(file.filename):
            ...
            with open(filename, 'rb') as f:
                for line in f:
                    ...
                    cur2 = g.db.execute('insert into users values (?,?,?)', [
...
def api_download_grades():
    data = []
    cur2 = g.db.execute(
        'select receiver, grader, grades from tasks natural join grades where creator = ?', [auth.username()])
...
def api_download_userinfo():
    ...
    for row in cur2.fetchall():
        data.append(
            {'username': row[0], 'password': row[1], 'privilege': row[2]})
...
def api_add_users():
    if not request.json:
        abort(400)
    cur2 = g.db.execute('insert into users values (?,?,?)', [
                        request.json['username'], request.json['password'], request.json['authorization']])
    g.db.commit()
...
def api_add_grades():
    if not request.json:
        abort(400)
    cur2 = g.db.execute('insert into grades values (?,?,?,?,?)', [
                        request.json['taskname'], request.json['grader'], request.json['receiver'], request.json['grades'], request.json['review']])
    g.db.commit()
...
def grading_stats():
    cur = g.db.execute('select taskname, grader, receiver, grades, review from tasks natural join grades where creator = ? and not grader = ? order by grades.taskname, receiver, grader', [
                       session['name'], session['name']])
    entries = [row for row in cur.fetchall()]
    # print entries
    cur1 = g.db.execute(
        'select taskname, receiver, grades, review from grades where grader = ? order by grades.taskname, receiver', [session['name']])
    ...
    cur2 = g.db.execute(
        'select taskname, weight from tasks where creator = ?', [session['name']])
    ...
    cur3 = g.db.execute(
        'select username from users where authorization = \'student\'')
...
def download():
    csv = "STUDENT, GRADE\n"
    cur = g.db.execute('select grades.taskname, grader, receiver, grades, review from tasks natural join grades where creator = ? and not grader = ? order by grades.taskname, receiver, grader', [
                       session['name'], session['name']])
    entries = [row for row in cur.fetchall()]
    # print entries
    cur1 = g.db.execute(
...
def grade_peers():
    cur = g.db.execute(
        'select taskname from student_task where student = ?', [session['name']])
    rows = cur.fetchall()
    if len(rows) > 0:
...
def add_grade():
    g.db.execute('delete from grades where taskname=? and grader=? and receiver=?', [
                 request.form['task'], session['name'], request.form['receiver']])
...
def manage_users():
    ...
    entries = [dict(username=row[0], privilege=row[2])
               for row in cur.fetchall()]
...
def grading_tasks():
    cur = g.db.execute(
        'select * from tasks where creator = ?', [session['name']])
    entries = [dict(taskname=row[0], creator=row[2], weight=row[1])
               for row in cur.fetchall()]
...
def edit_weight():
    if not session.get('logged_in'):
        abort(401)
    g.db.execute('update tasks set weight=? where taskname=? and creator=?', [
                 request.form['wt'], request.args.get('task'), request.args.get('creator')])
    g.db.commit()
...
def remove_task():
    if not session.get('logged_in'):
        abort(401)
    g.db.execute('delete from tasks where taskname = ?',
                 [request.args.get('task')])
    g.db.commit()
    g.db.execute('delete from student_task where taskname = ?',
                 [request.args.get('task')])
    g.db.commit()
    g.db.execute('delete from grades where taskname = ?',
                 [request.args.get('task')])
    g.db.commit()
    flash('New entry was successfully posted')
...
def block_task():
    if not session.get('logged_in'):
        abort(401)
    g.db.execute('delete from student_task where taskname = ? and not student =?', [
                 request.args.get('task'), session['name']])
    g.db.commit()
    flash('New entry was successfully posted')
